...
module.exports = {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (!selected) {
                    ...
                    sql = `SELECT * FROM tb_myjob as my, tb_job as j where my.jobId = j.id and deleted = 0 and my.userId = '${userId}' `;
                }
                if (name) {
    ...
    */
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (id) {
                    ...
                    let job = await this.getByName(name);
                    if (job) {
                        reject(1);
                        return;
                    }
    ...
    setMyJob(userId, jobs) {
        return new Promise(async (resolve, reject) => {
            try {
                let query = `delete from tb_myjob where userId=${userId}`;
                db.query(query, function (err, result) {
                    if (err) {
                        reject(err);
                    } else {
                        query = `insert into tb_myjob(userId,jobId) values ` +
                            jobs.map(item => {
                                return `(${userId}, ${item})`;
                            });
                        db.query(query, function (err, result) {
