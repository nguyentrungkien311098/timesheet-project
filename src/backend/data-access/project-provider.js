...
module.exports = {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (!selected) {
                    ...
                    sql = `SELECT * FROM tb_myproject as my, tb_project as pr where my.projectId = pr.id and deleted = 0 and my.userId = '${userId}' `;
                }
                if (name) {
    ...
    */
    ...
    setMyProject(userId, projects) {
        return new Promise(async (resolve, reject) => {
            try {
                let query = `delete from tb_myproject where userId=${userId}`;
                db.query(query, function (err, result) {
                    if (err) {
                        reject(err);
                    } else {
                        query = `insert into tb_myproject(userId,projectId) values ` +
                            projects.map(item => {
                                return `(${userId}, ${item})`;
                            });
                        db.query(query, function (err, result) {
