...
var productProvider = require("../data-access/product-provider");
//
const TB_TABLE = "tb_product";
const API_SERVICES = "/api/product/";
    ...
    return [
      ...
      () => {
        /* 
                ...
                */
        let jsonParser = bodyParser.json();
        app.put(API_SERVICES + "set-my-product", jsonParser, function(
          req,
          res
        ) {
          const user = authUtils.getUser(req.headers);
          if (user && user.user && user.user.id) {
            ...
            productProvider
              .setMyProduct(user.user.id, products || [])
              .then(s => {
                if (s.affectedRows !== 0) {
                  res.send(responseUtils.build(0, s));
                  return;
